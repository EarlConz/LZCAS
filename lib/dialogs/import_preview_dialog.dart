...
Future<List<int>?> showImportPreviewDialogWithSelection(
  ...
  return showDialog<List<int>>(
    ...
    builder: (ctx) {
      ...
      final displayCount = rows.length < previewRows
          ? rows.length
          : previewRows;
            ...
            content: SizedBox(
              ...
              child: Column(
                ...
                children: [
                        ...
                        onPressed: () {
                          setState(() {
                            for (var i = 0; i < selected.length; i++) {
                              if (!existsFlags[i]) selected[i] = true;
                            }
                          });
                        },
                        ...
                        rows: List.generate(displayIndices.length, (rowPos) {
                          ...
                          for (var i = 0; i < r.length; i++) {
                            ...
                            if (h.contains('time') ||
                                ...
                                h.contains('lastupdated')) {
                              final dt = _parseMaybeTimestamp(raw.toString());
                              formattedCells.add(
                                DataCell(Text(formatDisplayDate(dt))),
                              );
                            } else {
                  ...
                  const SizedBox(height: 8),
                  Text(
                    '${rows.length} total rows — showing ${displayIndices.length} rows — $selectedCount selected',
                  ),
                ],
              ),
            ),
