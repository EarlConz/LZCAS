...
class _SellButtonState extends State<SellButton> {
  ...
  void _showSellDialog(BuildContext context) {
    // Ensure data is fresh each time the dialog is opened
    _loadItems();
    _loadMembers();
    showDialog(
      context: context,
...
// A self-contained stateful widget for the dialog
...
class _SellDialogState extends State<_SellDialog> {
  ...
  void dispose() {
    ...
    // also dispose controllers in the cart
  ...
  bool isCartValid() {
    ...
    for (var entry in cart) {
      ...
      if (priceStr.isEmpty || (int.tryParse(priceStr) ?? 0) <= 0) {
        return false;
      }
    }
    return true;
            ...
            ElevatedButton.icon(
              onPressed: () async {
                if (selectedItem != null) {
                  final itemObj = (await repository.fetchItems()).firstWhere(
