-- ═══════════════════════════════════════════════════════════════════
-- Migration v13 — Upgrade promotes Member → Verified Reseller
--   Package availment is now the sole basis for reseller status
--   (see migration_v12). The upgrade RPC changed the package but never
--   the role, so a member upgrading from *no package* kept role='Member'
--   and login role='member'. Promote them (and sync the login account)
--   atomically inside the same locked transaction.
-- Supersedes the body of migration_v5_upgrade_guard.sql. Safe to re-run.
-- Requires is_staff() (from enable_rls_staff.sql).
-- ═══════════════════════════════════════════════════════════════════
...
begin
  ...
  --    no package (rank 0) to any package is a valid upgrade — this is the
  --    Member → Verified Reseller transition.
