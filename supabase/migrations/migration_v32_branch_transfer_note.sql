-- migration_v32_branch_transfer_note.sql
-- ---------------------------------------------------------------------------
-- Include the give-stock NOTE in the Reports movement reason.
-- Before: stock_movements.reason held only the branch cashier's name.
-- After:  reason = "<branch>" or "<branch> - <note>" when a note was entered,
--         so the note typed in "Give stock to a branch" shows up in Reports.
-- Applies to both give-out and return. Run AFTER migration_v31. Idempotent.
-- Reverse with rollbacks/rollback_branch_transfer_note_v32.sql.
-- ---------------------------------------------------------------------------
...
begin
  ...
  values (auth.uid(), p_item_id, coalesce(v_name, ''), p_quantity, 'transfer_in',
    case when p_note is not null and length(trim(p_note)) > 0
         then coalesce(v_username, 'branch') || ' - ' || p_note
         else coalesce(v_username, 'branch') end);
end;
$$;
